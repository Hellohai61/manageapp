...
1.main组件的实现，布局用的element-plus的container

[Container 布局容器 | Element Plus](https://element-plus.org/zh-CN/component/container.html)
...
> TIP
>
> 以上组件采用了 flex 布局，使用前请确定目标浏览器是否兼容。 此外， `<el-container>`的**直接子元素**必须是后四个组件中的一个或多个。 **后四个组件的父元素必须是一个 `<el-container>`**
...
<template>
    <!-- 左右布局，左侧自定义aside组件，右侧上面header，下面main区域 -->
...
先写大致结构，再引入动态数据，然后将动态数据通过v-for渲染
...
**思路：通过点击commonHeader组件按钮，传递数据到CommonAside中，控制左侧菜单el-menu的isCollapse属性**，涉及到组件传值，**用的pinia**

在components下的CommonAside.vue写代码

html中 ，给e-menu添加如下属性：

:collapse="isCollapse"，通过isCollapse控制是否折叠

![image-20250417151059670](C:\Users\HP\AppData\Roaming\Typora\typora-user-images\image-20250417151059670.png)



html中主要控制el-menu和 h3
...
//初始化state数据，这里我们使用一个函数来返回，里面如果有多个属性就更方便
function initState(){
  return {
    isCollapse:true,
...
给Header里面的button加点击事件handleCollapse

js中实现点击事件handleCollapse
...
3.在Main.vue中放置路由出口 <router-view></router-view>
...
我们通过封装 Axios 实例来**统一设置 baseURL、超时时间**、Content-Type 等，**避免在每个请求中重复写配置，提升开发效率。**

#### 2️⃣ **自动携带 Token，便于权限管理**

在**请求拦截器中统一添加用户 token，保证所有请求都能自动携带身份认证信息**，减少手动操作，同时方便管理权限接口。

------

#### 3️⃣ **统一响应格式处理，提升用户体验**

在**响应拦截器中可以统一判断后端返回的 code 值，对错误信息进行统一提示，比如用 Element Plus 的 ElMessage 弹窗，用户体验更友好。**

------

#### 4️⃣ **便于维护与升级**

将请求逻辑集中封装，比如错误处理、重试机制等逻辑都集中写在一个地方，一旦后期有改动，只需要维护一个入口即可。

------

#### 5️⃣ **支持多环境切换**

封装中**可以通过环境变量读取 baseURL，自动适配开发、测试、生产环境**，方便部署和调试。

------

#### 6️⃣ **封装 API 接口方法，调用更清晰**

我们通常会在封装 axios 的基础上进一步封装各模块的 API 请求方法，**调用的时候语义更清晰**、更易于维护。

------

##### ✅ 总结一句话：

二次封装 Axios 是实际项目开发中的标准做法，它让我们的请求**逻辑更统一、更安全、更易维护**，也更符合企业级项目的需求。
...
2.在src下创建api文件夹，在其中创建request.js**（重要）**
...
后端可能没有准备好数据，但是得先把请求数据的过程根据接口文档跑通，先制造假数据。
...
const getTableData=async ()=>{
  ...
  tableData.value=data.tableData  //一定注意ref需要.value获取其值
}
...
渲染订单统计卡片内容 home.vue中的template 以及写css样式
...
![image-20250420185040073](C:\Users\HP\AppData\Roaming\Typora\typora-user-images\image-20250420185040073.png)
...
回到home.vue中  （这是右边count部分+三个图表的html结构）
...
点击左侧菜单导航跳转到对应组件还没有实现

在CommonAside中给菜单elmenu添加点击事件@click="handleMenu(item)"
...
在CommonAside组件中继续补充handleMenu方法
